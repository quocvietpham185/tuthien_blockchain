...
export default function DonationModal({ campaign, account, onDonate, onClose }) {
  ...
  const remaining = Math.max(0, parseFloat(campaign.goal) - parseFloat(campaign.raised));
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal donation-modal" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="donation-campaign-info">
          <div className="donation-campaign-image">
            {getIPFSUrl(campaign.ipfsHash) ? (
              ...
              <div className="donation-image-placeholder">Ảnh</div>
            )}
          </div>
          <div>
        ...
        <div className="donation-actions">
          ...
          <button
            ...
            style={{ flex: 1 }}
          >
            {isLoading ? <><span className="spinner" /> Đang xử lý...</> : <>Xác Nhận Quyên Góp {amount ? `${amount} ETH` : ""}</>}
          </button>
        </div>
      </div>
    </div>
